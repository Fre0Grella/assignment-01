...
public class Boid {
    ...
    private P2d pos;
    private final ReadWriteLock rwlVel = new ReentrantReadWriteLock();
    private V2d vel;
    ...
    public void updateVelocity(BoidsModel model) {
        ...
        final var lock = this.rwlVel.writeLock();
        lock.lock();
    ...
    private V2d calculateAlignment(List<Boid> nearbyBoids, BoidsModel model) {
        ...
        if (nearbyBoids.size() > 0) {
	        for (Boid other : nearbyBoids) {
                final var lock = other.rwlVel.readLock();
                lock.lock();
            ...
            final var lock = this.rwlVel.readLock();
            lock.lock();
            final var finalVel = new V2d(avgVx - vel.x(), avgVy - vel.y());
            lock.unlock();
	        return finalVel.getNormalized();
        } else {
        	return new V2d(0, 0);
        }
    }
